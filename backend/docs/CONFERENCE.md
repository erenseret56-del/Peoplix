...
Conference calling defaults to disabled. The UI is available; access returns a friendly unavailable message until configured. No production credentials or provider agents are changed by installation.

1. Create a **dedicated Retell LLM agent** for the conference. Do not assign it to a customer, number profile, or the existing public demo. Choose an appropriate licensed voice in Retell.
2. Set its LLM general prompt to exactly `{{conference_instructions}}`. PEOPLIX injects the reviewed demo instructions from `src/modules/conference/conference.policy.ts` on every call. Leave knowledge bases and states empty; remove custom tools, customer lookups and integrations. An optional `end_call` tool is allowed. The backend verifies this configuration before issuing a call token and refuses a shared/customer agent.
3. Set the dedicated agent's maximum duration to **180,000 ms**, recording/transcript storage to **everything**, and signed recording URLs to enabled. PEOPLIX also sends per-call overrides for these properties. Configure all three webhook events: `call_started`, `call_ended`, `call_analyzed`.
4. Add only these **backend** variables:

   ```env
   CONFERENCE_ENABLED=true
   CONFERENCE_RETELL_AGENT_ID=your-dedicated-conference-agent-id
   CONFERENCE_RETELL_WEBHOOK_URL=https://YOUR_BACKEND/api/retell/webhook
   CONFERENCE_BLOCKED_EMAIL_DOMAINS=
   CONFERENCE_IP_SESSIONS_PER_MINUTE=600
   ```

5. Continue using the existing backend `RETELL_API_KEY`. No new browser secrets, Twilio numbers, customer accounts or separate admin credentials are needed. If the frontend/API have different origins, retain the existing `VITE_BASE_URL` and CORS setup. HTTPS is required for microphone access outside localhost. Route SPA refreshes to `index.html`.
6. Start the backend normally (`npm run build && npm start`). Startup creates additive indexes for `conference` and `conference_rate_limits`. Do not run a production server locally against production MongoDB merely to test this feature.
7. Perform a real staging voice call before enabling the conference publicly. Verify the v2 web-call overrides with the installed `retell-client-js-sdk` version, audio on iOS Safari and Android Chrome, all webhook deliveries and recording playback. The automated SDK double is not a provider compatibility test.
...
- The three-minute conversation budget starts when a call is reserved and includes connection setup. Retries share the original deadline and cannot extend it. At most three attempts are retained per session. Retell's per-call maximum duration is set from the remaining budget, capped at three minutes. New attempts with less than 75 seconds left are refused because Retell's minimum maximum-duration setting is one minute and setup needs time.
...
**300 simultaneous voice conversations are a target, not a guarantee.** The application shares its existing MongoDB pool, uses persistent state/leases across replicas, avoids periodic visitor API polling, code-splits the public page, and lazy-loads voice code only on Start Conversation. Browser audio goes directly to the provider, not through PEOPLIX API processes. Browser web calls do not use a Twilio phone number; any phone/SIP testing remains subject to Twilio capacity as well as Retell limits.

Confirm Retell concurrent-call allowance, API rate limits (including agent/LLM validation at call start), billing and published configuration. Verify Twilio capacity separately for any simultaneous phone demos. Size backend replicas and MongoDB using staging metrics, not synthetic success alone. Observe event delivery lag, deadline-worker lag, MongoDB pool waits, API p95/p99, 429s, failed registrations, missed recordings and microphone/audio errors.

`tests/conference-sessions.k6.js` exercises admission/status/end requests on an explicitly selected staging URL. It does not establish WebRTC conversations and cannot validate voice capacity. Run actual browser/provider voice load in controlled stages (25, 50, 100, 200, 300) with the account owner, dedicated staging agent and approved test participants. Include one shared-NAT test, two backend replicas, dropped webhooks, a backend restart, late token connection, stale/forged credentials, clock changes, and starting near the five-minute expiry.
...
Rollback: set `CONFERENCE_ENABLED=false` to block new conference access/calls. Keep backend workers running to finish existing calls and retain activity. No customer database migration or collection deletion is needed.
