...
export default function ConferenceEntry({ email, consent, busy, error, onEmailChange, onConsentChange, onSubmit }: ConferenceEntryProps) {
  ...
  const transition = { duration: reduced ? 0.18 : 0.85, ease: [0.76, 0, 0.24, 1] as const };
  ...
  return <motion.div className="conf-entry" data-exiting={!present} inert={!present} aria-hidden={!present}
    ...
    <div className="conf-entry-layout">
      <motion.header className="conf-entry-brand" variants={{ closed: { y: 0, opacity: 1 }, open: reduced ? { opacity: 0 } : { y: '-150%' } }} transition={transition}>
        <div className="conf-entry-beige" aria-hidden="true" />
      ...
      <motion.main className="conf-entry-bottom" aria-labelledby="conf-entry-title"
        variants={{ closed: { y: 0, opacity: 1 }, open: reduced ? { opacity: 0 } : { y: '120%' } }} transition={transition}>
        <div className="conf-entry-ivory" aria-hidden="true">
          <svg viewBox="0 0 1440 100" preserveAspectRatio="none"><path d="M0 0 C360 116 1080 116 1440 0 V100 H0Z" /></svg>
        </div>
        <div className="conf-entry-content">
          <p className="conf-entry-eyebrow">THE CONFERENCE EXPERIENCE</p>
          ...
          <form className="conf-form conf-entry-form" onSubmit={onSubmit} aria-busy={busy}>
            ...
            <button className="conf-primary conf-entry-submit" type="submit" disabled={busy}>
              <span>{busy ? 'Preparing your experience\u2026' : 'Meet Ava'}</span><ArrowRight size={19} strokeWidth={1.5} aria-hidden="true" />
            </button>
          ...
          <p className="conf-entry-headphones"><Headphones size={14} strokeWidth={1.4} aria-hidden="true" />Best with headphones</p>
        </div>
      </motion.main>
    </div>
  </motion.div>;
