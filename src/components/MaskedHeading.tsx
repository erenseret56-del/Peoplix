...
const MaskedHeading = ({
  ...
  // Main effect — resize observer + pointer parallax + idle drift
  useEffect(() => {
    const root = rootRef.current;
    if (!root) return;
    sync();
  ...
  // Re-sync when layout props change
  ...
  return (
    <Tag
      ...
      {...rest}
    >
      {/* Invisible measure layer — sets layout */}
      ...
      {/* Reveal layer — media clipped to text shape */}
